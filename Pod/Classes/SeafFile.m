...
@property (nonatomic, strong) SeafUploadFile *uploadTask;
...
- (instancetype)initWithModel:(SeafFileModel *)model connection:(SeafConnection *)connection {
    ...
    if (self) {
        ...
        _fileManager = [[SeafFileManager alloc] initWithConnection:connection];
...
#pragma mark - SeafFilePreviewHandlerDelegate

- (void)downloadWithCompletion:(void(^)(BOOL success, NSError *error))completion {
    if (self.isDownloading) {
        if (completion) {
            completion(NO, [NSError errorWithDomain:@"SeafFile" 
                                            code:-1 
    ...
    [self.fileManager downloadFile:self.model 
                      ...
                      completion:^(BOOL success, NSError *error) {
                          ...
                          if (success) {
                              [strongSelf.stateManager updateFileStatus:strongSelf.model 
                                                               state:nil 
                                                          localPath:strongSelf.model.localPath];
                              strongSelf.downloaded = YES;
                          }
                          
                          if (strongSelf.downloadCompletion) {
                              strongSelf.downloadCompletion(success, error);
                          }
                      }];
}
...
- (void)uploadWithPath:(NSString *)path completion:(void(^)(BOOL success, NSError *error))completion {
    ...
    self.uploadTask = [[SeafUploadFile alloc] initWithPath:path];
    self.uploadTask.delegate = self;
    [self.fileManager uploadFile:self.uploadTask];
}
