...
+ (nullable NSData *)composeMotionPhotoWithImageData:(NSData *)imageData
    ...
    return [self composeMotionPhotoWithImageData:imageData
                                       videoData:videoData
                                         options:@{@"presentationTimestampUs": @(presentationTimestampUs)}];
}
...
+ (BOOL)composeMotionPhotoWithImageData:(NSData *)imageData
    ...
    NSData *result = [self composeMotionPhotoWithImageData:imageData
                                                videoData:videoData
                                    presentationTimestampUs:presentationTimestampUs];
    
    if (!result) {
        if (error) {
            *error = [NSError errorWithDomain:@"SeafMotionPhotoComposer"
                                         code:-1
                                     userInfo:@{NSLocalizedDescriptionKey: @"Failed to compose Motion Photo"}];
        }
        return NO;
    }
    
    return [result writeToFile:outputPath options:NSDataWritingAtomic error:error];
}

#pragma mark - Advanced Composition
...
+ (nullable NSData *)composeMotionPhotoWithImageData:(NSData *)imageData
    ...
    // Extract options
    int64_t presentationTimestampUs = -1;
    if (options[@"presentationTimestampUs"]) {
        presentationTimestampUs = [options[@"presentationTimestampUs"] longLongValue];
    }
    
    // Determine image format
    BOOL isJPEG = [self isJPEGData:imageData];
    BOOL isHEIC = [self isHEICData:imageData];
    
    if (!isJPEG && !isHEIC) {
        Debug(@"SeafMotionPhotoComposer: Unsupported image format");
        return nil;
    }
    
    if (isJPEG) {
        return [self composeJPEGMotionPhoto:imageData 
                                 videoData:videoData 
                     presentationTimestampUs:presentationTimestampUs];
    } else {
        return [self composeHEICMotionPhoto:imageData 
                                 videoData:videoData 
                     presentationTimestampUs:presentationTimestampUs];
    }
}

#pragma mark - JPEG Motion Photo Composition

+ (nullable NSData *)composeJPEGMotionPhoto:(NSData *)jpegData
                                  videoData:(NSData *)videoData
                      presentationTimestampUs:(int64_t)presentationTimestampUs {
    // Generate XMP metadata
    NSString *xmpString = [SeafXMPHandler generateXMPStringWithPrimaryMime:@"image/jpeg"
                                                                 videoMime:[self mimeTypeForVideoData:videoData]
                                                               videoLength:videoData.length
                                                   presentationTimestampUs:presentationTimestampUs];
    NSData *xmpData = [xmpString dataUsingEncoding:NSUTF8StringEncoding];
    
    NSData *jpegWithXMP = jpegData;
    if (xmpData) {
        NSData *injected = [SeafXMPHandler injectXMPData:xmpData intoJPEGData:jpegData];
        if (injected) {
            jpegWithXMP = injected;
            Debug(@"SeafMotionPhotoComposer: XMP injected into JPEG successfully");
        }
    }
    
    // Compose: JPEG + Video
    NSMutableData *result = [NSMutableData dataWithCapacity:jpegWithXMP.length + videoData.length];
    [result appendData:jpegWithXMP];
    [result appendData:videoData];
    
    Debug(@"SeafMotionPhotoComposer: JPEG Motion Photo composed. Total: %lu bytes", (unsigned long)result.length);
    return [result copy];
}

#pragma mark - HEIC Motion Photo Composition (Standard Container Format)

+ (nullable NSData *)composeHEICMotionPhoto:(NSData *)heicData
                                  videoData:(NSData *)videoData
                      presentationTimestampUs:(int64_t)presentationTimestampUs {
    Debug(@"SeafMotionPhotoComposer: Building HEIC Motion Photo (Dropbox-compatible format)...");
    ...
    // Calculate video data size (for GCamera:MicroVideoOffset and Container:Directory)
    NSUInteger videoDataSize = videoData.length;
    
    // Generate XMP metadata (V1 format for compatibility, like Dropbox)
    NSString *xmpString = [SeafXMPHandler generateXMPStringWithPrimaryMime:@"image/heic"
                                                                 videoMime:@"video/mp4"
                                                               videoLength:videoDataSize
                                                   presentationTimestampUs:presentationTimestampUs];
    NSData *xmpData = [xmpString dataUsingEncoding:NSUTF8StringEncoding];
    
    Debug(@"SeafMotionPhotoComposer: Generated XMP metadata (%lu bytes)", (unsigned long)xmpData.length);
    
    // Parse iinf and iloc from meta box
    ...
    // We'll calculate the correct offset after determining actual meta size
    ...
    // Calculate actual meta size delta
    int64_t metaSizeDelta = iinfSizeDelta + ilocSizeDelta;
    ...
    Debug(@"SeafMotionPhotoComposer: Meta size delta: %lld (iinf: %lld, iloc: %lld)", 
          metaSizeDelta, iinfSizeDelta, ilocSizeDelta);
    ...
    // Only adjust items that are NOT the new XMP item
    for (SeafIlocItem *item in ilocData.items) {
        if (item.itemID == xmpItemID) {
            continue; // Skip XMP item, its offset is already correct
        }
        
        // Adjust base_offset if it points to or beyond original mdat
        if (item.baseOffset >= mdatBox.offset) {
            item.baseOffset += metaSizeDelta;
        }
        
        // Adjust extent offsets (if base_offset is 0, extent_offset is absolute)
    ...
    [newMetaContent appendData:[heicData subdataWithRange:NSMakeRange(metaPayloadStart, 4)]];
    ...
    for (SeafISOBMFFBox *child in metaBox.children) {
        if ([child.type isEqualToString:@"iinf"]) {
            ...
            [newMetaContent appendData:[heicData subdataWithRange:NSMakeRange(child.offset, child.size)]];
        }
    }
    ...
    Debug(@"SeafMotionPhotoComposer: Added meta (%u bytes)", metaSize);
    ...
    [result appendData:[heicData subdataWithRange:NSMakeRange(mdatBox.offset + 8, mdatBox.size - 8)]];
    ...
    Debug(@"SeafMotionPhotoComposer: Added mpvd (%lu bytes)", (unsigned long)mpvdBox.length);
    
    Debug(@"SeafMotionPhotoComposer: HEIC Motion Photo composed. Total: %lu bytes", (unsigned long)result.length);
