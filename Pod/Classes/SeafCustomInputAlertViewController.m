...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // Trigger keyboard presentation, which in turn triggers our animation
...
- (void)setupViews {
    ...
    self.alertView.layer.masksToBounds = YES; // clips to bounds for maskedCorners
...
- (void)setupConstraints {
    ...
    [NSLayoutConstraint activateConstraints:@[ // Full width, bottom constraint set above
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    [self confirmButtonTapped:nil]; // Simulate confirm button tap
    return YES;
}
...
- (void)presentOverViewController:(UIViewController *)presentingVC {
    [presentingVC presentViewController:self animated:NO completion:nil]; // Actual animation is handled by keyboard
}
