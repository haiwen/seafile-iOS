...
- (void)setupViews {
    self.view.backgroundColor = [UIColor clearColor]; // Ensure presenting VC is visible
    ...
    [UIView animateWithDuration:animationDuration delay:0.0 options:(animationCurve << 16) | UIViewAnimationOptionBeginFromCurrentState animations:^{
        ...
        if (self.isBeingDismissedProgrammatically) {
            [self dismissViewControllerAnimated:NO completion:^{
                // Check if the handler is the one we expect (the original self.handlerBlock)
                // AND if it was NOT a cancel operation
                ...
                self.currentDismissalWasCancel = NO; // Reset the new flag
            }];
        }
    }];
}
...
- (void)dismissAlertAndCallHandler:(HandlerBlock)handler isCancel:(BOOL)isCancel {
    ...
    self.currentDismissalWasCancel = isCancel; // Set the new flag
    ...
    if (self.nameTextField.isFirstResponder) [self.nameTextField resignFirstResponder];
        ...
        [self.view endEditing:YES]; // Try to dismiss keyboard
        // If keyboard wasn't up, or endEditing doesn't trigger hide notification:
        if (!self.alertView.alpha == 0) { // Check if already animating out
...
- (void)manuallyTriggerDismissAnimationIsCancel:(BOOL)isCancel {
    ...
    [UIView animateWithDuration:animationDuration delay:0.0 options:(animationCurve << 16) | UIViewAnimationOptionBeginFromCurrentState animations:^{
        ...
        // No need to check isBeingDismissedProgrammatically, this IS the dismissal
        [self dismissViewControllerAnimated:NO completion:^{
            if (!isCancel && self.handlerBlock) { // Only call handler if not a cancel
                NSString *name = self.nameTextField.text;
                NSString *pwd = self.encryptSwitch.isOn ? self.passwordTextField.text : nil;
                self.handlerBlock(name, pwd);
            }
...
- (void)encryptSwitchValueChanged:(UISwitch *)sender {
    BOOL przyszloIsOn = sender.isOn; // Capture the state
    [self updateEncryptionFieldsVisibility:przyszloIsOn animated:YES];

    // Delay changing the first responder until after the animation
    // The animation duration is 0.3s for showing, and 0.15s (fade) + 0.15s (layout) = 0.3s for hiding.
    // Use a slightly longer delay to be safe.
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (przyszloIsOn) {
            // Check if the switch is still on, in case it was toggled back quickly
            if (self.encryptSwitch.isOn) {
                 [self.passwordTextField becomeFirstResponder];
            }
    } else {
            // Check if the switch is still off
...
- (void)updateEncryptionFieldsVisibility:(BOOL)show animated:(BOOL)animated {
    if (animated) {
        if (show) {
            // Phase 1 for SHOW: Prepare for appearance (unhide, ensure alpha is initially 0)
            self.passwordTextField.hidden = NO;
            self.confirmPasswordTextField.hidden = NO;
            // Alpha should already be 0 from viewDidLoad or a previous hide animation's end state.
            // If rapidly toggling, ensure it starts from 0 for a clean fade-in.
            self.passwordTextField.alpha = 0.0;
            self.confirmPasswordTextField.alpha = 0.0;

            // Phase 2 for SHOW: Set constraints for 'shown' state
            ...
            // Phase 3 for SHOW: Layout *before* animation to establish positions
            ...
            [UIView animateWithDuration:0.3
                             ...
                             completion:nil];
        } else { // HIDING (animated)
            // Step 1: Animate alpha of password fields to 0 (fade out in place)
            [UIView animateWithDuration:0.15 // Duration for fade-out
                             animations:^{
                                 self.passwordTextField.alpha = 0.0;
                                 self.confirmPasswordTextField.alpha = 0.0;
                             }
                             completion:^(BOOL finishedFadeOut) {
                                 if (finishedFadeOut) {
                                     // Step 2: After fade-out, hide fields, change constraints, and animate layout shrinkage
                                     ...
                                     [UIView animateWithDuration:0.15 // Duration for layout shrinkage
                                                      animations:^{
                                                          [self.alertView layoutIfNeeded];
                                                      }];
                                 }
        ...
        if (show) {
            self.passwordTextField.alpha = 1.0;
            self.confirmPasswordTextField.alpha = 1.0;
            self.passwordTextField.hidden = NO; // Ensure visible if not animated
            self.confirmPasswordTextField.hidden = NO; // Ensure visible if not animated
        } else {
            self.passwordTextField.alpha = 0.0;
            self.confirmPasswordTextField.alpha = 0.0;
            self.passwordTextField.hidden = YES;
...
- (void)showAlertWithTitle:(NSString *)title {
    // Use the extension method if available and desired, or a simple UIAlertController
