...
- (void)showEditMenu:(UILongPressGestureRecognizer *)gestureRecognizer
{
    ...
    UIAlertController *alert = [self generateAlert:arr withTitle:nil handler:^(UIAlertAction *action) {
        ...
        if (index == 0) { // Edit
            ...
            SeafConnection *connection = [[SeafConnection alloc] initWithUrl:[SeafConnection generateUrlFromConnection:conn] cacheProvider:SeafGlobal.sharedObject.cacheProvider];
            
            // Set original information to conn's extended properties
            connection.originalUsername = conn.username;
            connection.originalAddress = conn.address;
            connection.address = conn.address;
            [connection.info setObject:conn.username forKey:@"username"];
            
            [self showAccountView:connection type:type];
...
- (UITableViewCell *)getAddAccountCell:(UITableView *)tableView
{
    ...
    cell.button.backgroundColor = SEAF_COLOR_ORANGE;
